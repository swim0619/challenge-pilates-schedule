...
def write_post(ed, post, log=print):
    _set_device_mode(ed, DEVICE_MODE, log=log)
    ...
    for idx, block in enumerate(post.blocks):
        try:
            if block.type == "heading":
                ...
                groups = _sentence_groups(block.text)
                for gi, group in enumerate(groups):
                    for line in group:
                        ed.type_rich(line)
                        ed.enter()
                    if BLANK_LINE and gi < len(groups) - 1:
                        ed.enter()   # 문장 사이 한 줄 띄기
                ...
                ed.set_bold(True)
                lines = _split_sentences(_strip_bold(block.text)) or [block.text]
                for j, line in enumerate(lines):
                    ed.type(("💡 " + line) if j == 0 else line)
                    if j < len(lines) - 1:
                        ed.enter()
                ed.set_bold(False)
                ed.enter(2)
...
def _lead_blank(ed, idx):
    """인용구 앞에 빈 줄을 넣어 앞 문단과 떼어 놓는다. 글 맨 앞에서는 넣지 않는다."""
    if idx > 0 and QUOTE_LEAD_BLANK > 0:
...
def _set_device_mode(ed, mode=DEVICE_MODE, log=print):
    """에디터 화면을 PC/모바일/태블릿 중 하나로 맞춘다.

    버튼 하나가 세 모드를 돌아가며 바꾸므로, 원하는 모드가 될 때까지 누른다.
    모바일로 두고 쓰면 실제 폰에서 보이는 줄바꿈 그대로 확인하면서 쓸 수 있다.
    """
    btn = ed.first_visible([S.DEVICE_MODE_BUTTON], timeout=4000)
    if btn is None:
        log("  · 화면 전환 버튼을 찾지 못했습니다 (PC 화면으로 진행)")
        return False
    for _ in range(4):
        cls = btn.get_attribute("class") or ""
        if S.DEVICE_MODE_CLASS + mode in cls:
            log("· 에디터 화면: %s" % mode)
            return True
        try:
            btn.click(timeout=3000)
            ed.page.wait_for_timeout(1200)
        except Exception:
            break
    log("  · %s 화면으로 바꾸지 못했습니다" % mode)
    return False
...
def _insert_place(ed, query, log=print):
    ...
    try:
        ...
        # 검색어와 전혀 다른 곳이 첫 결과로 잡히는 일이 있다(동명이인 상호).
        # 검색어의 낱말이 하나도 안 겹치면 넣지 않고 알린다.
        words = [w for w in query.split() if len(w) > 1]
        if words and not any(w in found for w in words):
            raise RuntimeError(
                "검색 결과 '%s' 가 검색어 '%s' 와 맞지 않아 넣지 않았습니다" % (found, query))
...
# 말이 자연스럽게 끊기는 자리 — 쉼표와 연결어미
_CLAUSE_END = re.compile(
    r"(?:,|은데|는데|지만|니까|라서|아서|어서|해서|이고|하고|으면|면서|"
    r"다가|거나|든지|는지|았고|었고|어도|아도|고요|구요)$"
)
# 세 글자 이상인 말의 '~고 / ~서 / ~며' 끝맺음도 끊어 읽기 좋은 자리
...
def _sentence_groups(text, lo=LINE_MIN, hi=LINE_MAX):
    """문단을 '문장 덩어리'로 나눈다. 덩어리 사이에 한 줄을 띄우면 숨통이 트인다.

    한 줄짜리 짧은 문장이 연달아 나오면 같은 덩어리로 묶는다.
    문장마다 무조건 띄우면 너무 뚝뚝 끊겨 보이기 때문이다.
    """
    groups = []
    for sent in [x.strip() for x in _SENT_RE.split(_glue_bold(text.strip())) if x.strip()]:
        lines = [l.replace(_GLUE, " ") for l in _flow_lines(sent, lo, hi)]
        if groups and len(lines) == 1 and len(groups[-1]) == 1:
            groups[-1].extend(lines)
        else:
            groups.append(lines)
    return groups
...
def _split_sentences(text, lo=LINE_MIN, hi=LINE_MAX):
    """문단을 모바일에서 읽기 좋은 줄로 나눈다 (한 줄 띄기 없이 평평하게)."""
    return [l for g in _sentence_groups(text, lo, hi) for l in g]
