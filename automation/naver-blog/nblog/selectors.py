...
# PC / 모바일 / 태블릿 화면 전환 버튼.
# 클래스의 se-util-button-device-<모드> 가 '현재' 모드를 뜻하고,
# 누를 때마다 desktop -> mobile -> tablet -> desktop 으로 돈다.
DEVICE_MODE_BUTTON = "button.__mode-button"
DEVICE_MODE_CLASS = "se-util-button-device-"
...
QUOTE_STYLES = {
    "따옴표": "default",                  # 인용구1
    "버티컬라인": "quotation_line",         # 인용구2 (왼쪽 세로선)
    "말풍선": "quotation_bubble",          # 인용구3
    "라인&따옴표": "quotation_underline",   # 인용구4 (따옴표 + 아래 라인)
    "포스트잇": "quotation_postit",         # 인용구5
    "프레임": "quotation_corner",           # 인용구6
    "코너": "quotation_corner",            # 인용구6 (같은 스타일, 옛 이름)
}
