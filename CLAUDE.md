...
- Each class card shows the member's pass as a plain "잔여 N회" (remaining count) badge, read directly off `activePasses[0].remaining_sessions` — there used to be a computed per-class "session number" (n회차) badge, but it repeatedly confused the owner when a class was marked absent (absent classes are excluded from the count, so the numbering didn't move the way people expected), so it was removed in favor of just showing the actual remaining count.
